...
export function LandSection() {
  ...
  const stats = [
    { value: '40+', label: t('stats.farms') },
    { value: '120', label: t('stats.farmers') },
    { value: '8', label: t('stats.provinces') },
    { value: '65%', label: t('stats.direct') },
  ];
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-10 self-start">
            {stats.map((stat) => (
              <div key={stat.label} className="border-l-2 border-gold pl-4">
                <p className="font-display text-4xl tracking-tight text-forest md:text-5xl">
                  {stat.value}
                </p>
